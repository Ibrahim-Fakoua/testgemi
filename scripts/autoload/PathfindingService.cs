...
public partial class PathfindingService : Node
{
    ...
    /// <summary>
    /// Returns an array of unique Big Hex coordinates (areas) that cover all currently used tiles.
    /// </summary>
    private List<Vector2I> DesignateAreas()
    {
        var usedCells = (_tilemap as TileMapLayer).GetUsedCells();
        
        // HashSet automatically filters out duplicates. 
        // If 61 individual tiles all belong to area (0,1), it only gets added once.
        var uniqueAreas = new HashSet<Vector2I>();

        foreach (var cell in usedCells)
        {
            // 1. Convert the 2D map coordinate (Axial) to 3D Cube coordinates
            Vector3I cubeTile = AxialToCube(cell);

            // 2. Find exactly which Big Hex Area this tile mathematically belongs to
            Vector2I parentArea = SmallHexToBig(cubeTile);

            // 3. Add to our unique list
            uniqueAreas.Add(parentArea);
        }

        return uniqueAreas.ToList();
    }
...
private void FixAdjacencyForChunks(List<Vector2I> areasToCheck)
    {
        foreach (var areaVector in areasToCheck)
        {
            ...
            // Safety check to ensure the area was actually found and within bounds
            if (index >= _areasPos.Count || _areasPos[index] != areaVector)
            {
                GD.PushError($"Failed to find area at {areaVector} during adjacency fix.");
                continue;
            }
            ...
            // Replaced chunkQueue with a properly scoped list
            var neighboringChunks = new List<Chunk>();

            // Collect all chunks from neighboring areas exactly ONCE per area
            foreach (var neighboringArea in neighbors)
            {
                ...
                if (actualNeighboringAreaIndex < _areasPos.Count &&
                    _areasPos[actualNeighboringAreaIndex] == neighboringArea2i)
                {
                    var actualNeighboringArea = _areas[actualNeighboringAreaIndex];
                    neighboringChunks.AddRange(actualNeighboringArea.Chunks);
                }
            }

            // Iterate through each chunk in the current area
            for (int i = 0; i < area.Chunks.Count; i++)
            {
                var initialChunk = area.Chunks[i];

                // 1. Check connections to other chunks within the SAME area
                // We start at i + 1 to prevent self-checking and redundant double checks (A->B and B->A)
                for (int j = i + 1; j < area.Chunks.Count; j++)
                {
                    var destinationChunk = area.Chunks[j];
                    if (!initialChunk.AreYouMyNeighbor(destinationChunk))
                    {
                ...
                // 2. Check connections to chunks in NEIGHBORING areas
                foreach (var destinationChunk in neighboringChunks)
                {
                    if (!initialChunk.AreYouMyNeighbor(destinationChunk))
                    {
                        if (AreChunksAdjacent(initialChunk, destinationChunk))
    ...
    {
        return MoveToClosestThing(origin, gdTags, null);
    }
    public Variant MoveToClosestThing(Vector3I origin, Godot.Collections.Array gdTags,
        Godot.Collections.Array gdFilter)
    {
        var tags = ToStringArray(gdTags);
        var filter = gdFilter != null ? ToStringArray(gdFilter) : Array.Empty<string>();

        // 1. We now get the actual chunk that contains the target
        var foundChunk = SearchForThings(origin, 12, tags, filter);

        if (foundChunk != null)
        {
            // 2. Find the actual target object inside that chunk
            ...
            if (thing.VariantType != Variant.Type.Nil)
            {
                ...
                // 3. Pathfind DIRECTLY to the target, completely ignoring chunk boundaries!
                var path = Pathfind(point1, point2);

                if (path != null && path.Length > 0)
                {
                    // If length is 1 or 2, we are standing on or immediately next to the prey
                    if (path.Length <= 2)
                    {
                        return thing; 
                    }
                    ...
                    {
                        // Take one step along the direct shortest path to the prey
    ...
    {
       return AmIInDanger(startingPosition, fightscore, morale, null);
    }
    public Variant AmIInDanger(Vector3I startingPosition, int fightscore, int morale,
        Godot.Collections.Array gdFilter )
    {
    ...
    private Chunk SearchForThings(Vector3I startingPosition, int maxDepth, string[] things, string[] filter)
    {
        ...
        if (chunk != null)
        {
            ...
            if (results.Count > 0)
            {
                // Return the chunk that ACTUALLY contains the prey
                finalChoice = results[GD.RandRange(0, results.Count - 1)];
            }
    ...
    public Area GetAreaFromTile(Vector3I tile)
    {
        if (ValidateTile(tile))
        {
            ...
            if (index < _areas.Count && _areasPos[index].Equals(areaCoord))
            {
                // Found an exact match for the area coordinate.
                return _areas[index];
            }
            GD.PushError("Area not found for tile: " + tile); // Log error
        }
        else
        {
            GD.PushError("Tile does not exist: " + tile); // Log error
        }
    ...
    public Chunk GetChunkAtTile(Vector3I tile)
    {
        ...
        if (area != null)
        {
            ...
            GD.PushError("Chunk not found for tile: " + tile + " within area."); // Log error
        }
        // If area is null, GetAreaFromTile already logged an error and returned null.
        // So, we just need to return null here.

        return null; // Return null instead of throwing exception
    }

    /// <summary>
    /// Finds an entity at a specific tile location.
    /// </summary>
    /// <param name="tile">The tile coordinates to search.</param>
    /// <returns>The GodotObject found at the tile, or null if no entity is found.</returns>
    public GodotObject FindEntityAtTile(Vector3I tile)
    {
        var chunk = GetChunkAtTile(tile);
        if (chunk != null)
        {
            foreach (var thing in chunk.Contents)
            {
                if (thing.Get("position").As<Vector3I>() == tile)
                {
                    return thing;
                }
            }
    ...
    public partial class Area : RefCounted
    {
        ...
        public void RebuildChunks(Vector2I areaVector, List<Vector3I> tilesInArea)
        {
            ...
            {
                // 1. Grab the first tile and IMMEDIATELY remove it to completely 
                // eliminate the risk of an infinite while-loop.
                ...
                // 2. Safely check if the tile exists and if it is impassable.
                // Using .As<bool>() prevents casting crashes if the property is missing.
                ...
                if (tileData == null )
                {
                    ...
                    continue; // Tile is invalid or impassable; skip to the next
                    
                }

                if (tileData.GetCustomData("Impassable").As<bool>())
                {
                    i++;
                    j++;
                    _remainingTiles.RemoveAt(0);
                ...
                if (results.Count > 0)
                {
                    ...
                    // 3. Remove the newly explored tiles from the remaining list.
                    // Using RemoveAll is more performant than a foreach loop with Remove().
                    _remainingTiles.RemoveAll(tile => results.Contains(tile));
                }
                else
                {
                    // Changed from Error to Warning so it doesn't break debugger flow, 
                    // since the infinite loop threat is now resolved.
                    GD.PushWarning($"CubeExplore returned no tiles to explore for tile at {startTile}.");
                    // To prevent an infinite loop, we must remove the tile that caused the issue.
                    _remainingTiles.Remove(startTile);
                }
            }
            Console.WriteLine("NumberOfCellsSkiped = " + i + "Impassable " + j);
        }
