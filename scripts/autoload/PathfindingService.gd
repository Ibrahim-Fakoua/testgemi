...
func move_to_closest_thing(origin : Vector3i, tags : Array[String],filter : Array[String] = []):
	...
	if found_chunk:
		...
		if origin_chunk == found_chunk and origin_chunk != null:
			var thing : GenericEntity = found_chunk.find_nearest_thing(tilemap,origin,tags,filter)
			if thing != null:
...
func get_nearest_prey(origin : Vector3i, food_chain : int, filter : Array[String] = []):
	...
	if chunk:
		...
		push_error("invalid chunk for finding prey")
...
func search_for_things(starting_position : Vector3i,max_depth : int, things : Array[String], filter : Array[String]):
	...
	if chunk:
		...
		push_error("Invalid chunk for searching")

func find_entity_at_tile(tile):
	var chunk = get_chunk_at_tile(tile)
	if chunk:
		for thing : GenericEntity in chunk.contents:
			if thing.position == tile:
				return thing
	return null
...
class Chunk:
	...
	func find_nearest_thing(tilemap : MainLayer, origin : Vector3i, searched_tags, filter:Array[String]=[]):
		var Distance = 9999
		var chosen_thing
		for thing : GenericEntity in get_all_with_tags(origin,searched_tags, filter):
