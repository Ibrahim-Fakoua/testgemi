...
public partial class DataGraph<T> : Control, IGraph where T : DataQuery
{
...
public partial class PopulationGraph : DataGraph<PopulationRow>
{
	private Dictionary<int, Color> _speciesColors = new();
	private const float Padding = 40f;
	
	public bool ShowBirthMarkers { get; set; } = false;
	public bool ShowDeathMarkers { get; set; } = false;
	...
	private void DrawDataSeries(ScaleData scale, List<PopulationRow> series, Color color, Font font)
	{
		...
		foreach (var point in sorted)
		{
			...
			if (ShowBirthMarkers) DrawFlowIndicator(scale, pos, point.Births, Colors.Green, "+", goesUp: true, font);
			if (ShowDeathMarkers) DrawFlowIndicator(scale, pos, point.Deaths, Colors.Red, "-", goesUp: false, font);
	...
	public void SetShowBirthMarkers(bool value)
	{
		ShowBirthMarkers = value;
		QueueRedraw();
	}

	public void SetShowDeathMarkers(bool value)
	{
		ShowDeathMarkers = value;
		QueueRedraw();
	}
}

public partial class SpeciesActionPieChart : DataGraph<ActionCountRow>
{
	private readonly Color[] _actionColors = {
	...
	public override void _Draw()
	{
		...
		float radius = Mathf.Min(center.X, center.Y) * 0.7f;

		int total = 0;
		foreach (var row in _data) total += row.Count;
		if (total == 0) return;
		...
		{
			float sweepAngle = (float)row.Count / total * Mathf.Tau;
			var color = _actionColors[colorIndex % _actionColors.Length];
    
			// Build polygon points: center + arc points
			var points = new List<Vector2> { center };
			int segments = 64;
			for (int s = 0; s <= segments; s++)
			{
				float angle = startAngle + sweepAngle * s / segments;
			...
			var labelPos = center + new Vector2(Mathf.Cos(midAngle), Mathf.Sin(midAngle)) * (radius * 1.1f);
			DrawString(ThemeDB.FallbackFont, labelPos, row.LogAction, HorizontalAlignment.Center, -1, 16, Colors.White);
