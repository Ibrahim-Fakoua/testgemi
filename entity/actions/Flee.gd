...
func activate() -> void:
	var possible_danger = critter.pathfinding.am_i_in_danger(critter.position, critter.get_fightscore(), critter.get_morale(), [critter.get_species()])
	if possible_danger :
		startup()
		critter.move(critter.pathfinding.move_away_from(critter.position, possible_danger))
	else :
		end()
