...
func remove_events_of_type(event_type:String) -> void :
	cleanup_ordered_events(event_type)
	var typed_events = ordered_events[event_type]
	for i in range(typed_events.size() - 1, -1, -1) :
		events.erase(ordered_events[event_type][i])
		typed_events.remove_at(i)
