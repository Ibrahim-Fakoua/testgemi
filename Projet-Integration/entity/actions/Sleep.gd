...
func startup() -> void :
	...
	time_until_completely_rested = -critter.get_time_until_fatigue(0) * critter.attributes.sleep_efficiency
...
func get_fatigue_recovery() -> int :
	...
	return time_sleeping + time_sleeping/critter.attributes.sleep_efficiency
...
func recover_fatigue(is_waken_up_by_force:bool) -> void :
	...
	if !critter.scheduled_events.get_events().has(critter.scheduled_events.find_by_id("coma")) and time_sleeping == time_until_completely_rested:
		...
		var fatigue_recovery = time_sleeping + time_sleeping/critter.attributes.sleep_efficiency
