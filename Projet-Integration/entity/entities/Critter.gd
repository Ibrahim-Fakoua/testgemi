...
func _schedule_startup_events(litter_size:int) -> void :
	...
	var starting_food_percentage:float = (1.0 / 3.0 / (litter_size as float)) + 0.1
	...
	var time_to_adulthood = floor(get_mating_cooldown() * 5)
...
func _initialize_default_state_machine(specie:SpecieData.Species) -> void :
	...
	var coma = State.new([Sleep.new(self)], self)
	...
	add_state_change(wandering, FatigueReached.new(90), coma)
	add_state_change(seeking_food, FatigueReached.new(90), coma)
	add_state_change(seeking_love, FatigueReached.new(90), coma)
	add_state_change(hunting, FatigueReached.new(90), coma)
	add_state_change(fleeing, FatigueReached.new(90), coma)
	
	add_state_change(coma, FatigueUnder.new(30), wandering)
...
func get_hunger_percentage() -> float :
	...
	print((1 - time_until_starvation / total_time_of_starvation) * 100)
...
func give_birth() -> void :
	...
	for i in possible_birth_locations :
		if count < attributes.litter_size:
			count = count+1
			Critter.new(specie, position, pathfinding, attributes.litter_size)
		decrease_hunger(-(CritterConst.BASE_STARVATION_TIME * attributes.hunger_rate * 0.4))
