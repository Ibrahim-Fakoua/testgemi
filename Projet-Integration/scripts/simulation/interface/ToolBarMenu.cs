...
public partial class ToolBarMenu : MenuBar
{
	...
	public void InitializeFromConfig(string jsonPath)
	{
		...
		var menuData = JsonSerializer.Deserialize<Dictionary<string, List<ToolbarOption>>>(jsonContent);
	...
	private void PopulateSubMenu(PopupMenu parentPopup, List<ToolbarOption> subMenu)
	{
		foreach (var option in subMenu)
		{
			...
			if (option.Type?.ToLower() == "separator" || option.Label == "-")
			{
	...
	private void OnItemIdPressed(long id)
	{
		...
		{
			Controller.Instance.Emit(new UISignal(methodName));
		}
	}

	/*
	 * Method called by the master interface script to delegate the 'heartbeat'
	 */
